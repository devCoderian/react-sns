...
const PostImages = ({ images }) => {
  ...
  if (images.length === 1) {
    return (
      <>
        <img role="presentation" src={`http://localhost:3060/${images[0].src}`} alt={images[0].src} onClick={onZoom} />
        {showImagesZoom && <ImagesZoom images={images} onClose={onClose} />}
      </>
    );
  }
  if (images.length === 2) {
    return (
      <>
          <img role="presentation" style={{ width: '50%', display: 'inline-block' }} src={`http://localhost:3060/${images[0].src}`} alt={images[0].src} onClick={onZoom} />
       <img role="presentation" style={{ width: '50%', display: 'inline-block' }} src={`http://localhost:3060/${images[1].src}`} alt={images[1].src} onClick={onZoom} />
  ...
  return (
    <>
      <div>
        ...
        <img role="presentation" style={{ width: '50%' }} src={`http://localhost:3060/${images[0].src}`} alt={images[0].src} onClick={onZoom} />
        <div
          role="presentation"
          style={{ display: 'inline-block', width: '50%', textAlign: 'center', verticalAlign: 'middle' }}
          onClick={onZoom}
